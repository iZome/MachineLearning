...
task2i <- function()
    {
      ...
      m_error <- avg_runs_i(40, N, sigma) ## choose number of runs, N and sigma
...
task2ii <- function()
    {
      ...
      m_error <- avg_runs_ii(20, N, Q_f, sigma) ## choose number of runs, N, Q_f and sigma
...
## if matrix is stored in table format with named columns and rows, it can be loaded from disk and plotted
...
## (optional) plot target function with nois together with fitted models, good for viewing if the function fits are good or not
plot_run <- function(
    x_values,
    y_values,
    x_test_data,
