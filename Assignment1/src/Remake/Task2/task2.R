...
calculate_e_out_2 <- function(number_new, beta_set_tf, beta_set_g, Q_tf, sigma){
    #comparison_beta_set <- beta_set_tf - beta_set_g
    new_x_values <- generate_uniform_values(number_new)
    new_y_values <- targetFunction2(new_x_values, beta_set_tf, Q_tf) + rnorm(n = length(new_x_values), mean = 0, sd = sigma**2)
    new_fit_values <- targetFunction2(new_x_values, beta_set_g, Q_tf)
    mse <- mean((new_y_values - new_fit_values)**2)
    #print(mse)
    return(mse)
}
...
calculate_e_out <- function(x_tf, y_tf, beta_set_tf, Q_g, sigma){
    ...
    # return (bias_g$value)
    error_mse <- calculate_e_out_2(30, beta_set_tf, beta_set_g, length(beta_set_tf) - 1, sigma)
    return(error_mse)
}

find_overfit <- function(x_tf, y_tf, beta_set_tf, Q_set, sigma){
    err_g_1 <- calculate_e_out(x_tf, y_tf, beta_set_tf, Q_set[1], sigma) #g_1 is first g, in this task g2
    err_g_2 <- calculate_e_out(x_tf, y_tf, beta_set_tf, Q_set[2], sigma) #g_2 is second g, in this task g10
...
make_error_matrix_i <- function(N_set, sigma_set, beta_set_tf, Q_tf){
    ...
    for(k in 1:length(N_set)){
        ...
        for(l in 1:length(sigma_set)){
            ...
            overfit_measure <- find_overfit(x_tf, y_tf, beta_set_tf, c(2,10), sigma)
            #to get smud results set all values above 0.2 or below -0.2 to +- 0.2 (probably not right?)
...
avg_runs_i <- function(Q_tf, n_averages, N_set, sigma_set){
    ...
    for(i in 1:n_averages){
        ...
        #Live plot, open pdf with reader that refresh
        pdf("error_matrix.pdf")
        image(avg_matrix, zlim = c(-0.2,0.2), col = colorRamps::matlab.like(100))
        dev.off()
        pdf("3D-plot2.pdf")
        persp3D(x = N_set, y = sigma_set, z = avg_matrix, colvar = avg_matrix , clim = c(-0.2,0.2), zlim = c(-0.2,0.2))
        dev.off()
    }
    close(pb)
    return(avg_matrix)
